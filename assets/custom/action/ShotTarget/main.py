...
class ShotTarget(CustomAction):
    def run(
        ...
        water = context.run_recognition("检查子弹_custom", image,{"检查子弹_custom":{"roi":[1029,653,25,33]}})
        empty = context.run_recognition("检查子弹_custom", image,{"检查子弹_custom":{"roi":[1107,653,24,33]}})
        ...
        if has_eject and is_not_shot_chance_true and has_bullets:
            ...
            if eject_count >= bullet_count-1:
                print("开始退弹")
                while (not int(water.best_result.text) ==0) and (not int(empty.best_result.text) ==0):
                    x,y = (
